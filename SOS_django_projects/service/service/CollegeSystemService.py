from service.utility.DataValidator import DataValidator
from .BaseService import BaseService
from django.core.paginator import Paginator
from service.models import CollegeSystem

'''
It contains Role business logics.   
'''


class CollegeSystemService(BaseService):

    def search(self, params):

        page_no = int(params.get("page_no", 0))
        page_size = self.pageSize

        query = self.get_model().objects.all()

        if page_no == 0:
            return query

        value = params.get("student_name", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(student_name__istartswith=value.strip())

        value = params.get("branch", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(branch__istartswith=value.strip())

        value = params.get("semester", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(semester__istartswith=value.strip())

        value = params.get("cgpa", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(cgpa__istartswith=value.strip())

        paginator = Paginator(query, page_size)

        page_obj = paginator.get_page(page_no)

        params["has_next"] = page_obj.has_next()
        params["has_previous"] = page_obj.has_previous()
        params["start_index"] = (page_no - 1) * page_size

        return page_obj

    def get_model(self):
        return CollegeSystem
