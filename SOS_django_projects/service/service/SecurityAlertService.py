...
class SecurityAlertService(BaseService):
    ...
    def search(self, params):
        ...
        value = params.get("threat_level", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(threat_level__istartswith=value.strip())

        value = params.get("source_ip", None)
        if (DataValidator.isNotNull(value)):
            query = query.filter(source_ip__istartswith=value.strip())
